...
@registration.callback_query(F.data=="авто")
async def Filterr(callback: CallbackQuery,state:FSMContext):
...
async def reg_grope(callback: CallbackQuery, state: FSMContext):
    if callback.data!="+ДОБ ГРУППУ":
        ...
        if (fname_in_db(str(data["name"]))) and ("#" not in str(data["name"])) and ("|" not in str(data["name"])) and (")" not in str(data["name"])) and ("(" not in str(data["name"])) and len(str(data["name"]))<=13:
            ...
            await callback.message.edit_text(data["name"] + "-это имя занято, слишком большое или содержит недопустимые символы",
                                             reply_markup=kBackmebu)
        await state.clear()
    else:
        await state.clear()
        await state.set_state(set_grop.grope)
        await callback.message.edit_text("отправьте название группы в скобках\nпример:(PM2) или (ПМ2)", reply_markup=sozd_grop)
...
async def reg_grope(callback: CallbackQuery, state: FSMContext):
    ...
    if (")" in data["grope"]) and ("(" in data["grope"]):
        ...
        await bot1.send_message(chat_id="1442714637", text=f'создать группу{data["grope"]} от |{callback.message.chat.first_name}|{callback.message.chat.username} id#{callback.message.chat.id}#',reply_markup=prow_sozd)
        print(f'создать группу{data["grope"]} от |{callback.message.chat.first_name}|{callback.message.chat.username} id#{callback.message.chat.id}#')
    else:
...
def fgen_spisok_grop():
    ...
    c.append([InlineKeyboardButton(text="назад ↩️",callback_data="menu")])
...
kreg_name = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="проверить имя ✅",callback_data="reg_name")],
                                                [InlineKeyboardButton(text="Автоматически-имя в тг",callback_data="авто")],
